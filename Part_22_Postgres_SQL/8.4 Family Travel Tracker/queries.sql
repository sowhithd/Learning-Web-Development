...
/*
 Many to Many relationship to represent data that can have a Many to One relationship on both sides.
...
CREATE TABLE enrollment (
  ...
  /*
    ...
    * So if we had Student 1 enrolled in Class 2, we should not have two duplicate records where we 
      have another one which represents Student 1 enrolled in Class 2.
    
    * That's what that primary key part does, and ensures that each of the records in the 
       enrollment table are unique.
  */
...
Select std.first_name || ' ' || std.last_name as fullname ,cla.title from enrollment enr
join student std on enr.student_id = std.id
join class cla on cla.id = enr.class_id
